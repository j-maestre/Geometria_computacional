...
esat::Vec3 puntos1[10];
esat::Vec3 puntos2[10];
void InitStar(){
    float angle = 6.28f / 10;

    for (int i = 0; i < kNPoints; i++){
        puntos1[i].x = (float) cos(angle * i);
        puntos1[i].y = (float) sin(angle * i);
        puntos2[i].x = (float) cos(angle * i);
        puntos2[i].y = (float) sin(angle * i);
